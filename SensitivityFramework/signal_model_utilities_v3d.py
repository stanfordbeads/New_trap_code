# this one uses 8um attractor and is identical otherwise to 3b
...
# sine
def position_at_time_sin_function(stroke,time,frequency,offset_y=0,phase_offset=0): 
    '''
    ...
    '''
    pos_at_time = stroke/2*np.sin(2*np.pi*time*frequency+phase_offset)+offset_y
    return pos_at_time

## sinusoidal movement
def force_at_a_time_sin_function(stroke,time,frequency,pos_vec,force_vec,offset_y=0,phase_offset=0):
    '''
    Interpolates between the position to get a smooth force vs time for a sine function
    '''
    osci_pos = position_at_time_sin_function(stroke,time,frequency,offset_y=offset_y,phase_offset=phase_offset)
    return np.interp(osci_pos,pos_vec,force_vec, left=None, right=None, period=None)

def force_vs_time(x_sep,height_sep,stroke,frequency,lambda_val,axis,offset_y=0,phase_offset=0,yuk_or_grav="yuk",alpha=1,bead_size=7.6):
    '''
    ...
    force = alpha * force_at_a_time_sin_function(stroke,time,frequency,p,f,offset_y=offset_y,phase_offset=phase_offset)
    return time,force